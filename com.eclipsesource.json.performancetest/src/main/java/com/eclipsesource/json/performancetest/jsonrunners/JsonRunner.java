...
public abstract class JsonRunner {
  ...
  {
    // We use StreamEncoder so we can use flushBuffer().  We don't like flush() because that also calls flush()
    // on the underlying output stream, which is something we don't want.
    // See: JsonRunner_Test.testWriteToOutputStream_doesNotFlushOutputStream
    sun.nio.cs.StreamEncoder writer = sun.nio.cs.StreamEncoder.forOutputStreamWriter( out, new Object(), UTF8 );
